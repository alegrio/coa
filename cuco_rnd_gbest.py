...
class Nest:
    ...
    def levy_flight(self, gbest):
        """
        Aplica o voo de Lévy para os ovos do ninho usando o gbest fornecido.
        """
        if gbest is None:
            return  # Não faz nada se o gbest não foi definido
        ...
        for egg in self.eggs:
            ...
            levy_step = (u / np.abs(v) ** (1 / beta)) * (gbest.function_parameters - egg.function_parameters)
...
class Farmer:
    def __init__(self, population_size, eggs_number, abandon_rate, env: Environment):
        ...
        self.gbest = None  # Adiciona o gbest como atributo do Farmer
    ...
    def select_gbest(self):
        """
        Seleciona um ovo aleatório dentre os Pareto ótimos como gbest.
        """
        if self.pareto_optimal:
            self.gbest = random.choice(self.pareto_optimal)
        else:
            self.gbest = None
...
def main(max_generation, population_size, eggs_number, abandon_rate, dimension, lower_bound, upper_bound, fitness_function):
    ...
    # Cria o ambiente e inicializa o Farmer
    ...
    for i in range(max_generation):
        ...
        alegrio.rank_eggs()
        alegrio.select_gbest()  # Seleciona o gbest aleatório entre os Pareto ótimos
    ...
    # Gera o gráfico final e salva os resultados
    if len(alegrio.pareto_optimal) > 0:
        alegrio.save_pareto_to_excel()  # Salva os indivíduos em pareto_optimal
        Graph_controller.plot_sch(alegrio.pareto_optimal)  # Plota o gráfico com os Pareto ótimos
    else:
        print("Not enough Pareto optimal solutions to save.")
