...
class Nest:
    ...
    def levy_flight(self, gbest):
        ...
        levy_step = (u / np.abs(v) ** (1 / beta)) * (gbest.function_parameters - self.function_parameters)
...
class Farmer:
    ...
    def save_pareto_to_excel(self, filename="pareto_optimal.xlsx"):
        ...
        """
        data = []
        for nest in self.pareto_optimal:
            row = list(nest.fitness) + list(nest.function_parameters)
            data.append(row)
    ...
    def abandon_by_rate(self):
        """
        Abandona os ninhos com base no abandon_rate e preenche a lista com novos ninhos.
        O número total de ninhos após o abandono será igual ao population_size.
        """
        num_to_abandon = int(len(self.nests) * self.abandon_rate)  # Calcula o número de ninhos a abandonar
        self.nests = self.nests[:-num_to_abandon]  # Remove os últimos ninhos
        num_to_create = self.population_size - len(self.nests)  # Calcula quantos ninhos novos criar
        self.nests.extend(self.create_nests(num_to_create))  # Adiciona novos ninhos para manter o tamanho original
...
def main(max_generation, population_size, abandon_rate, dimension, lower_bound, upper_bound, fitness_function):
    ...
    if len(alegrio.nests) > 0:
        ...
        Graph_controller.plot_zdt1(alegrio.nests)  # Plota todos os ninhos
    else:
